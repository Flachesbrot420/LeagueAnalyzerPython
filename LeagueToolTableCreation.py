from LeagueTool import Base, DBSession, engine
from tg import expose, TGController
from sqlalchemy import Boolean, Float, create_engine, Column, Integer, String, ForeignKey, BigInteger, JSON
...
class TimelineDto(Base):
    ...
    meta_id      = Column(Integer, ForeignKey('metadata_timeline.id'))  # renamed from metadata_id
    info_id      = Column(Integer, ForeignKey('info_timeline.id'))

    meta = relationship('MetadataTimeLineDto', back_populates='timeline', uselist=False)  # renamed from metadata
...
class MetadataTimeLineDto(Base):
    ...
    timeline = relationship('TimelineDto', back_populates='meta', uselist=False)  # back_populates updated
...
# ── ParticipantFramesDto (container 1-9) ──────────────────────────────────────
...
class MatchDto(Base):
    ...
    id      = Column(Integer, primary_key=True, autoincrement=True)
    meta_id = Column(Integer, ForeignKey('match_metadata.id'))
    info_id = Column(Integer, ForeignKey('match_info.id'))

    meta = relationship('MatchMetadataDto', back_populates='match', uselist=False)
    info = relationship('MatchInfoDto',     back_populates='match', uselist=False)
...
class MatchMetadataDto(Base):
    ...
    dataVersion  = Column(String)
    matchId      = Column(String)
    participants = Column(JSON)  # List[string]

    match = relationship('MatchDto', back_populates='meta', uselist=False)
...
class MatchInfoDto(Base):
    ...
    match        = relationship('MatchDto',        back_populates='info', uselist=False)
...
class ParticipantDto(Base):
    ...
    id                              = Column(Integer, primary_key=True, autoincrement=True)
    info_id                         = Column(Integer, ForeignKey('match_info.id'))
    challenges_id                   = Column(Integer, ForeignKey('match_challenges.id'))
    missions_id                     = Column(Integer, ForeignKey('match_missions.id'))
    perks_id                        = Column(Integer, ForeignKey('match_perks.id'))
    ...
    playerScore10                   = Column(Integer)
    playerScore11                   = Column(Integer)
    ...
    challenges = relationship('ChallengesDto',   back_populates='participant', uselist=False)
    missions   = relationship('MissionsDto',     back_populates='participant', uselist=False)
    perks      = relationship('PerksDto',        back_populates='participant', uselist=False)
...
class LeagueListDTO(Base):
    ...
    entries = relationship('LeagueItemDTO', back_populates='league_list')


# ── LeagueItemDTO ─────────────────────────────────────────────────────────────

class LeagueItemDTO(Base):
    __tablename__ = 'league_item'

    id             = Column(Integer, primary_key=True, autoincrement=True)
    league_list_id = Column(Integer, ForeignKey('league_list.id'))
    ...
    league_list  = relationship('LeagueListDTO',  back_populates='entries')
    mini_series  = relationship('MiniSeriesDTO',  back_populates='league_item', uselist=False)
...
class MiniSeriesDTO(Base):
    ...
    league_item  = relationship('LeagueItemDTO',  back_populates='mini_series',  uselist=False)
